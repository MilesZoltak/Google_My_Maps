...
private const val REQUEST_CODE_CREATE = 123
private const val REQUEST_CODE_DISPLAY = 321
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapAdapter = MapsAdapter (this, userMaps, object: MapsAdapter.OnClickListener {
            override fun onItemClick(position: Int) {
                ...
                intent.putExtra(EXTRA_MAP_TITLE, userMaps[position].title)
                intent.putExtra(EXTRA_MAP_INDEX, position)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CREATE) {
                ...
                //remove the map data from the data
                val userMap = data?.getSerializableExtra(EXTRA_USER_MAP) as UserMap
                val toRemove = data.getIntExtra(EXTRA_MAP_INDEX, -1)
                Log.i(TAG, "map index: $toRemove")
                userMaps.removeAt(toRemove)
                mapAdapter.notifyItemRemoved(toRemove)
                serializeUserMaps(this, userMaps)
            }
        }
